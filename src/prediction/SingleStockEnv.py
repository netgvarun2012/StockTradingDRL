...
class SingleStockEnv(gym.Env):
    ...
    def _sell_stock(self, index, action):
        """Function emulating sell behaviour by updating different state variables"""
        action = int(action)
        self.state[index+STOCK_DIM+3] += abs(action)
        self.state[index+STOCK_DIM+1] -= abs(action)
        self.trades+=1
        ...
        elif self.state[index+STOCK_DIM+3] >=100:   # goes into this if sell_trades>2 (since action is always between -100 to 100).
            self.state[index+STOCK_DIM+1] = self.state[index+STOCK_DIM+3]-100+self.state[index+STOCK_DIM+1]
            self.state[index+STOCK_DIM+3] = 100
            
    def _buy_stock(self, index, action):
        """Function emulating buy behaviour by updating different state variables"""
        action = int(action)
        curr_num_shares = self.state[index+STOCK_DIM+1]     # -25
        self.state[index+STOCK_DIM+1] +=  action # -25+26=>1
        self.trades+=1
        self.buy_trades+=1
        self.buying_prices.append(self.data.adjcp)
        curr_num_shares_bought =  self.state[index+STOCK_DIM+2]  #75
        
        self.state[index+STOCK_DIM+2] += action  #101
        if self.state[index+STOCK_DIM+2] >= 100 and self.buy_trades>=2:
            self.state[index+STOCK_DIM+1] = curr_num_shares + 100-max(3-self.buy_trades,0)-curr_num_shares_bought#-25+100-0-75  #-25+100-0
    ...
    def step(self, actions):
        """Function receives action as input and calls corresponding sell/buy functions."""
    ...
    def reset(self):
        """Function resets the state variables to initial value."""
        self.asset_memory = [INITIAL_ACCOUNT_BALANCE]
        self.day = 0
        self.data = self.df.loc[self.day,:]
        self.cost = 0
